...
/**
 * Script to upload pattern textures to Cloudflare R2
 * Supports: PNG, JPG, JPEG, WEBP formats
 * 
 * Usage:
 *   pnpm tsx scripts/uploadPatterns.ts <patterns-directory>
 * 
 * Example:
 *   pnpm tsx scripts/uploadPatterns.ts ./assets/patterns
 */
async function uploadPatterns(patternsDir: string) {
  try {
    console.log('📦 Starting pattern upload to R2...\n');
    ...
    if (!patternsDir) {
      throw new Error('Please provide a patterns directory path');
    }
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (supportedFormats.includes(ext)) {
            ...
            const r2Key = `textures/patterns/${relativePath.replace(/\\/g, '/')}`;
    ...
    const absolutePath = join(process.cwd(), patternsDir);
    ...
    if (uploadedFiles.length > 0) {
      ...
      console.log(`   ${process.env.CLOUDFLARE_R2_PUBLIC_URL}/textures/patterns/`);
    }
...
// Get patterns directory from command line argument
const patternsDir = process.argv[2];

if (!patternsDir) {
  console.error('❌ Error: Please provide a patterns directory path');
  console.error('Usage: pnpm tsx scripts/uploadPatterns.ts <patterns-directory>');
  console.error('Example: pnpm tsx scripts/uploadPatterns.ts ./assets/patterns');
  process.exit(1);
}

uploadPatterns(patternsDir);
